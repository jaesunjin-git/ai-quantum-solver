"""
engine/result_interpreter.py — v2.0
철도 승무 도메인 결과 해석기 + 하위 호환 공개 API.

솔버가 반환한 원시 솔루션(x[i,d], u[d], s[d], e[d] 등)을
듀티 배정표, KPI 요약, 제약 만족 현황으로 해석합니다.

v2.0 변경사항:
  - RailwayResultInterpreter가 GenericResultInterpreter를 상속
  - 목적함수 분류·제약 라벨을 result_mapping.yaml에서 로드
  - 공개 함수 interpret_result() / save_artifacts()는
    get_interpreter(domain)을 통해 도메인별 인터프리터에 위임
  - 기존 호출부 변경 불필요 (하위 호환)
"""
